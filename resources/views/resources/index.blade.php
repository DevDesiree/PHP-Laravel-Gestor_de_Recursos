...
<div class="ml-24 w-3/12">
    <div class="mt-4 container-search w-">
        ...
        <input type="text" name="search" id="search" class="mt-1 p-2 border rounded-md w-full" placeholder="Escribe para buscar...">
    </div>
...
<div class="flex flex-row flex-wrap gap-10 px-12 pt-4 w-auto mb-10 justify-center">
    ...
    <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 resource-card">
        ...
        <div class="flex flex-col items-center gap-2 pb-10">
            <img class="w-24 h-24 mb-3 rounded-full shadow-lg object-cover object-center" src="{{ url(isset($resourceTypeImages[$resource->resourcetype->titleResource]) ? $resourceTypeImages[$resource->resourcetype->titleResource] : '/img/default.png') }}" alt="resource image" />
            ...
            <h5 class="mb-1 text-lg px-4 text-center font-small font-bold {{ isset($resourceTypeColors[$resource->resourcetype->titleResource]) ? $resourceTypeColors[$resource->resourcetype->titleResource] : '' }}">
                {{$resource->resourcetype->titleResource }}
            </h5>
            <h5 class="mb-1 text-lg px-4 text-center font-small text-gray-900 dark:text-white {{ isset($themeColors[$resource->theme->title]) ? $themeColors[$resource->theme->title] : '' }}">
                {{$resource->theme->title }}
            </h5>
            <a href="{{ $resource->extraResource->url }}" class="mb-1 text-xxl px-4 text-center font-small text-gray-900 dark:text-white hover:text-blue-500">
